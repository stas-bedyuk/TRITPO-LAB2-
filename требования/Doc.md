...
График — это линия, наглядно отражающая характер зависимости выбранных величин друг от друга.
...
Цель ПО – обеспечить перевод нескольких валют в одну единую с помощью одного клика. А также вывод актуальной информации о курсах валют и прорисовка графиков.

 ## 4 ХАРАКТЕРИСТИКА ОБЪЕКТА АВТОМАТИЗАЦИИ
 
Программа обеспечивает перевод четырех валют (доллар, евро, белорусский рубль, российский рубль) в одну единую валюту (по выбору пользователя). Также программа обеспечивает вывод актуальной информации о курсах валют и выводит график зависимости одной валюты от другой (по выбору пользователя).
...
Данный программный продукт  на выходе  должен соответствовать мокапам.
...
При запуске программы  появиться окно "МЕНЮ", где у пользователя появится возможность выбрать одно из действий (Калькулятор, курсы валют, графики). При выборе действия "Калькулятор" программа должна спарсить курсы валют и затем перевести данные пользователя (денежную массу) в определенную валюту (которую выбрал пользователь). При выборе действия "курсы валют" программа должна вывести актуальную информацию в соотвествии с выбором пользователя пары валют. При выборе действия "графики" программа должна изобразить график зависимости пары валют в соответствии с выбором пользователя пары валют.
...
Проработать все возможные варианты неправильного ввода данных. Также требуется предусмотреть случаи внезапного отключения от сети интернет и официального сайта нацбанка.

Проверить правильность работы калькулятора.
